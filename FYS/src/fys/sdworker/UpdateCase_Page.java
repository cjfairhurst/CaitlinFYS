...
/**
 * This class enables service-desk-workers to update cases.
 *
...
public class UpdateCase_Page extends javax.swing.JPanel {
    ...
    /**
     * Loads a case from the database based on the selected case.
     * 
     * @param rs received from the database.
     * @throws SQLException 
     */
    ...
    private void initComponents() {
        ...
        view_panel.add(edit_button, new org.netbeans.lib.awtextra.AbsoluteConstraints(650, 470, -1, -1));
        ...
        view_panel.add(PDF_button, new org.netbeans.lib.awtextra.AbsoluteConstraints(480, 470, -1, -1));
        ...
        if (confirm == 0) {
            if (client_radiobutton.isSelected()) {
                if (FYS.getQueryManager().checkConnection(selectedRow)) {
                    ...
                    FYS.getQueryManager().createLog(TrackUser.getCurrentUser(), "UpdateCase_Page",
                            "Deleted Client Case " + selectedRow + ".");

                    FYS.getQueryManager().deleteCase("client", selectedRow);

                    FYS.getInstance().updateTable(updatecase_table,
                            FYS.getQueryManager().fillTableCase("client"));
                }
            } else if (baggage_radiobutton.isSelected()) {
                if (FYS.getQueryManager().checkConnection(selectedRow)) {
                    JOptionPane.showMessageDialog(null, "This case cannot be deleted because it's "
                    ...
                    FYS.getInstance().updateTable(updatecase_table,
                            FYS.getQueryManager().fillTableCase("baggage"));
                }
            }
        }
    ...
    private void save_buttonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_save_buttonMouseClicked
        ...
        emptyBaggageField = baggageid_textfield.getText().equals("")
                ...
                && brand_textfield.getText().equals("")
                && color_textfield.getText().equals("")
    ...
    private void addconnection_buttonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_addconnection_buttonMouseClicked
        FYS.getInstance().updateTable(clientcase_table,
                FYS.getQueryManager().fillTableCase("client"));

        FYS.getInstance().updateTable(baggagecase_table,
                FYS.getQueryManager().fillTableCase("baggage"));
    ...
    private void saveconnection_buttonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_saveconnection_buttonMouseClicked
        if (clientTableClicked && baggageTableClicked) {
            FYS.getQueryManager().createLog(TrackUser.getCurrentUser(), "UpdateCase_Page",
                    "Added connection between Client Case " + selectedClientRow
                    + " and Baggage Case " + selectedBaggageRow + ".");
            ...
            JOptionPane.showMessageDialog(null,
                    "Please select both a client case and a baggage case from the tables above.",
                    "Selection Error", JOptionPane.INFORMATION_MESSAGE);
        }
